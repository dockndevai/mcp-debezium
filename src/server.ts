...
import { ConnectClient, ConnectError } from "./connect/client.js";
...
export function buildServer(config: AppConfig): { server: McpServer; enabled: string[] } {
  ...
  const server = new McpServer({ name: "mcp-debezium", version: "0.1.3" });
...
function toErrorResult(err: unknown) {
  ...
  if (err instanceof PolicyError) {
    ...
    message = `Kafka Connect error (${err.status}): ${err.message}${err.body ? `\n${truncate(err.body)}` : ""}`;
